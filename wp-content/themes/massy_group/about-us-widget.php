...
class aboutus_widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			'aboutus_widget',
			// widget name
			__('About Us Widget', ' aboutus_widget_domain'),
			// widget description
			array( 'description' => __( 'About us block in Who we are section', 'aboutus_widget_domain' ), )
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		echo '<div class="col-md-6 about_side" data-aos="slide-right" data-aos-offset="200">
	      <h3 class="sec_subHead">'.$title.'</h3>
	      <div class="row">
	        <div class="col-md-6 pr-0">
	          <img
	            src="'.get_template_directory_uri().'/assets/images/aboutImage.png"
	            alt="about image"
	            class="sectionImage"
	          />
	        </div>
	        <div class="col-md-6 pl-0">
	          <div class="d-flex flex-column align-items-center justify-content-between content_right h-100">
	            ...
	            <a href="'.get_permalink( get_page_by_path($instance['url']) ).'" class="learnMore">learn more</a>
	          </div>
	        </div>
	      </div>
	    </div>';
	...
	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) )
			...
			$title = __( 'Default Title', 'aboutus_widget_domain' );
		if ( isset( $instance[ 'desc' ] ) )
			$desc = $instance[ 'desc' ];
		else
			$desc = __( 'Default Desc', 'aboutus_widget_domain' );
		if ( isset( $instance[ 'url' ] ) )
			$url = $instance[ 'url' ];
		else
			$url = __( '#', 'aboutus_widget_domain' );
		?>
		<p>
