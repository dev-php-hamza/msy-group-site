...
class MyAutoloader
{
    ...
    {
    	if(strpos($className, 'FinancialDataEntry') !== false){
	    	if (strpos($className, 'Lib') !== false) {
	    		$class = str_replace('\\', '/', $className);
	    		$class_array = explode('/', $class);
			 	$location =  WP_PLUGIN_DIR .'/financial-data-entry/lib/'.$class_array[2] . '.php';  
			 	if (is_file($location)) {
	...
	public static function widgetLoader($className)
    {
    	if(strpos($className, 'FinancialDataEntry') !== false){
	    	if (strpos($className, 'Widgets') !== false) {
	    		...
	    		$class = str_replace('\\', '/', $className);
	    		$class_array = explode('/', $class);
			 	$location =  WP_PLUGIN_DIR .'/financial-data-entry/widget/'.$class_array[2] . '.php';  
			 	if (is_file($location)) {
		        	require_once($location);
		    	} 
			}
...
spl_autoload_register('MyAutoloader::widgetLoader');
?>